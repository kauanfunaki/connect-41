...
import { sendTesteDiscEmail } from "@/lib/email/sendMail";
...
export async function gerarLinkTeste(
  ...
  let emailSent = false;
  if (person.email) {
    const result = await sendTesteDiscEmail({ tenantId: ctx.tenantId, to: person.email, personName: person.name, token });
    emailSent = result.ok;
  }
