...
// self-check parses.
//
// forked from design-sync lib/bundle.mjs — this repo's components pull in
// real `next/link`/`next/navigation` runtime code (not just types), which
// references many `process.env.__NEXT_*` flags esbuild's narrow
// `process.env.NODE_ENV` define never matches. Browsers have no `process`
// global at all, so every unmatched read threw ReferenceError at first
// render, breaking 100% of previews. Only `sharedBuildOptions`'s `define`
// changed (a shim object covering all `process.*` reads) — the bundle
// header/manifest contract in bundleToIife/stampHeader is untouched.
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  return {
    ...
    minify: false,
    // forked: broadened from the upstream `{'process.env.NODE_ENV': ...}`
    // — see the header comment for why (raw Next.js runtime code reads many
    // process.env.__NEXT_* flags that specific key never matches, and plain
    // `process` doesn't exist in a browser at all).
    // esbuild define values must be a JS literal (no function expressions),
    // so process.nextTick is left unset — nothing on the render path calls it.
    define: {
